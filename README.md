...
| **Storeys** | A storey is a whole plan with a floor-to-floor height; the level below is ghosted under the one you are drawing, and the 3D view stacks them. A flight of stairs cuts its own stairwell in the floor above, on the plan as well as in 3D. The eye button on the storey switcher shows one storey at a time, which is also the only state where what you see and what you can click are the same set. A design with one storey saves exactly the file it always did. |
| **Roof** | Settings &rarr; Roof: flat, gable or hip over the building, with a pitch and an eaves overhang. Off by default. A ceiling is still drawn inside each room &mdash; a ceiling and a roof are different things. |
| **Surfaces** | Click a wall or a floor: pick an image, then tint it, turn it and slide it. A room's ceiling gets a tint too. The tint is a multiply, so white means untinted rather than white &mdash; and clearing it puts the file back exactly as it was. Normal and roughness maps draw under the Studio look; the tint draws under both. |
...
| **3D** | <kbd>F</kbd> walk through · <kbd>E</kbd> exterior view · <kbd>O</kbd> orthographic · <kbd>G</kbd> wireframe. The exterior view frames the whole building — every storey and the roof — rather than the storey being edited. |
