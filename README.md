...
| **Storeys** | Behind a flag while programme G finishes (`levelsEnabled`). A storey is a whole plan with a floor-to-floor height; the level below is ghosted under the one you are drawing, and the 3D view stacks them. A design with one storey saves exactly the file it always did. |
